...
#        return x
#    try:
#        print(pretrain_df.sort_values('Delta_E'))
#    except:
#        print('no pretrain')
#    newTdf = pd.read_pickle('data-from-polydisp-with-PR/database-DxyzrqE-T{}_M3_xyzQ.pickle'.format(Tlab))
#    for m in range(1,M):
#        x = newTdf['x_%d'%m] - newTdf['x_0']
#        y = newTdf['y_%d'%m] - newTdf['y_0']
#        z = newTdf['z_%d'%m] - newTdf['z_0']
#        # PBC
#        x = x.apply(PBC_dist)
#        y = y.apply(PBC_dist)
#        z = z.apply(PBC_dist)
#    
#        newTdf['0to%d'%m] = np.sqrt( x*x + y*y + z*z ) 
#        newTdf = newTdf.drop(columns=['x_%d'%m,'y_%d'%m,'z_%d'%m,'r_%d'%m, 'dpow2_%d'%m, 'dpow4_%d'%m])
#        #newTdf = newTdf.drop(columns=['x_%d'%m,'y_%d'%m,'z_%d'%m,'r_%d'%m, 'dpow2_%d'%m, 'dpow4_%d'%m, 'wrong_dpow4_%d'%m])
#    m=0
##    newTdf = newTdf.drop(columns=['x_%d'%m,'y_%d'%m,'z_%d'%m,'r_%d'%m, 'dpow2_%d'%m, 'dpow4_%d'%m, 'wrong_dpow4_%d'%m])
#    newTdf = newTdf.drop(columns=['x_%d'%m,'y_%d'%m,'z_%d'%m,'r_%d'%m, 'dpow2_%d'%m, 'dpow4_%d'%m])
#    for i in range(0,M):
#        for q in range(2,14):
#            newTdf=newTdf.drop(columns=['q{}_{}'.format(q,i)])
#    newTdf['T']=T
#    newTdf['conf']='NotAvail'
#    newTdf['i']='NotAvail'
#    newTdf['j']='NotAvail'
#    pretrain_df = pd.concat([pretrain_df,newTdf])
#    print(pretrain_df.sort_values('Delta_E'))
#    print(pretrain_df.columns)
#    pretrain_df.to_pickle('MLmodel/pretraining-qs-regression-M{}.pickle'.format(M))
#    sys.exit()
##################################################################################
##################################################################################
